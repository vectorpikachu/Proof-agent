...
The input task will follow the template below.
...
You can use the following tool to help understand the proof status.

### Tool: `get_definition`

When you are unclear with some definitions in the current state, or you are interested in some definition, you can call the function `get_definition` to the get the definition of a list of identifiers.

#### Tool Input

A list of Coq identifiers, e.g., `[Nat.add, List.app, canonical]`

#### Tool Execution

For each identifier `id` in the input list, the tool appends the tactic `Print id.` into the coq script, invokes `coqc`, and returns the result.